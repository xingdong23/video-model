...
- `cli.py`: 命令行入口
- `engine.py`: 可复用接口
- `pipeline.py`: 已迁移到 `my-video` 的 TuiliONNX 推理逻辑
- `faces/`: 可选的本地预设人物视频
- `output/`: 默认输出目录
...
- `my-video/digital_human/models/tuilionnx/checkpoints`
- `my-video/digital_human/faces`

默认目录：

- `/Users/xingdong/workspace/HD_HUMAN/my-video/digital_human/models/tuilionnx/checkpoints`
- `/Users/xingdong/workspace/HD_HUMAN/my-video/digital_human/faces`
...
.venv/bin/pip install -r digital_human/requirements.txt
```
...
```bash
cd /Users/xingdong/workspace/HD_HUMAN/my-video
.venv/bin/python -m digital_human.cli faces
```
...
```bash
cd /Users/xingdong/workspace/HD_HUMAN/my-video
.venv/bin/python -m digital_human.cli generate \
...
.venv/bin/python -m digital_human.cli generate \
  ...
  --face 测试视频.mp4 \
  --runtime auto \
  --output digital_human/output/output.mp4
```
...
- 默认只读取 `my-video/digital_human` 自己目录下的模型和人物素材
...
- `digital_human/cli.py`
- `digital_human/engine.py`
- `digital_human/config.py`
- `digital_human/pipeline.py`
- `digital_human/__init__.py`
- `digital_human/models/tuilionnx/checkpoints/`
- `digital_human/faces/`
...
```bash
cd /path/to/parent/of/digital_human
python -m digital_human.cli --help
```

如果你不在 `digital_human` 的父目录下执行，也可以显式指定：

```bash
PYTHONPATH=/path/to/parent/of/digital_human python -m digital_human.cli --help
```
...
如果后面需要，我可以继续把“字幕/BGM/封面/发布”也按 `my-video` 模块化方式拆出来。
