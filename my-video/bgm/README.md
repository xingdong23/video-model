...
它不依赖旧项目里的 Gradio、`utils/voice_processor.py`、固定 `social-auto-upload-main/videos/output.mp4` 路径，也不要求你保留原有工程目录结构。复制 `my-video/bgm` 到别的项目后，只要机器上有 `ffmpeg` 和 `ffprobe`，就能直接使用。
