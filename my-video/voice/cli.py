...
if TYPE_CHECKING:
    if __package__ in (None, ""):
        from voice.engine import VoiceEngine  # type: ignore
    else:
...
def _load_create_app():
    if __package__ in (None, ""):
        from voice.server import create_app  # type: ignore
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Standalone CosyVoice wrapper for my-video")
...
def main() -> None:
    ...
    output = Path(args.output).expanduser().resolve() if args.output else engine.paths.voices_dir / f"{args.voice_name}.pt"
