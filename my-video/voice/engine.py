...
class VoiceEngine:
    def __init__(self, model_dir: str | os.PathLike | None = None, fp16: bool = True):
        ...
        os.environ["COSYVOICE_CUSTOM_VOICE_DIR"] = str(self.paths.voices_dir)
    ...
    def list_custom_speakers(self) -> List[str]:
        if not self.paths.voices_dir.exists():
            return []
        return sorted(path.stem for path in self.paths.voices_dir.glob("*.pt"))
    ...
    def export_custom_voice(
        ...
        destination = (
            ...
            if output_path
            else (self.paths.voices_dir / f"{normalized_name}.pt").resolve()
        )
