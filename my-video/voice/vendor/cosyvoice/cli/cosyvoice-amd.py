...
class CosyVoice:
    ...
    def inference_sft(
        ...
        # 先获取所有分段
        text_segments = list(self.frontend.text_normalize(tts_text, split=True))
        ...
        if raw_speeches:  # 确保有音频数据
            ...
            temp_audio_path = os.path.join(target_dir, "post_request_test.wav")
            torchaudio.save(
                temp_audio_path,
                final_audio,
                24000,  # 保持原始采样率
    ...
    #
    #             # 只在最后一个分段处理完成后保存完整音频
    #             if i == list(self.frontend.text_normalize(tts_text, split=True))[-1]:
    #                 # 合并所有音频片段
    #                 final_audio = torch.cat(raw_speeches, dim=1)
    #
    #                 # 保存完整音频
    ...
    #                     24000,  # 保持原始采样率
    #                 )
    #                 logging.info(f"已保存完整音频，包含 {len(raw_speeches)} 个片段")
    #
    #             yield model_output
