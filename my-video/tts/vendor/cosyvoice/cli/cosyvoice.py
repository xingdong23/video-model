...
# 设置环境变量以避免Intel SVML相关的LLVM错误
# os.environ['NUMBA_DISABLE_INTEL_SVML'] = '1'
# os.environ['MKL_ENABLE_INSTRUCTIONS'] = 'SSE4_2'
# os.environ['OMP_NUM_THREADS'] = '1'
# os.environ['NUMBA_DISABLE_AVX'] = '1'
# os.environ['NUMBA_DISABLE_AVX2'] = '1'
# os.environ['NUMBA_DISABLE_AVX512'] = '1'
# os.environ['NUMBA_DISABLE_AVX512F'] = '1'
# os.environ['NUMBA_DISABLE_FASTMATH'] = '1'
...
def _resolve_model_dir(model_dir):
    ...
    if snapshot_download is None:
        raise FileNotFoundError(
            f"model dir not found: {model_dir}. Install modelscope or pass a local model directory."
        )
    return snapshot_download(model_dir)


def _resolve_speaker_path(spk_id):
    speaker_dir = os.environ.get(
        "COSYVOICE_CUSTOM_VOICE_DIR",
        os.path.join(grandparent_dir, "speakers"),
    )
    return os.path.join(speaker_dir, f"{spk_id}.pt")
...
def time_it(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        logging.info(f"推理方法 {func.__name__} 运行时长: {duration:.4f} 秒")
        return result
...
class CosyVoice:
    ...
    def inference_sft(
        ...
        logging.info(f"文本共分为 {len(text_segments)} 个片段")

        # 遍历所有文本分段
        for i in tqdm(text_segments):
            logging.info(f"text_normalize方法处理之后的文本片段: {i}")
            if spk_id not in DEFAULT_VOICES:
                model_input = self.frontend.frontend_sft(i, "中文女")
                speaker_path = _resolve_speaker_path(spk_id)
                newspk = torch.load(speaker_path, map_location="cpu")
                model_input["flow_embedding"] = newspk["flow_embedding"]
                model_input["llm_embedding"] = newspk["llm_embedding"]
                model_input["llm_prompt_speech_token"] = newspk[
                    "llm_prompt_speech_token"
                ]
                model_input["llm_prompt_speech_token_len"] = newspk[
                    "llm_prompt_speech_token_len"
                ]
                model_input["flow_prompt_speech_token"] = newspk[
            ...
            for model_output in self.model.tts(
                ...
                raw_speeches.append(audio_data.clone())

        # 所有分段处理完成后，合并并保存完整音频
        if raw_speeches:  # 确保有音频数据
            logging.info(f"开始合并 {len(raw_speeches)} 个音频片段")
            final_audio = torch.cat(raw_speeches, dim=1)
            logging.info(f"已保存完整音频，总长度: {final_audio.shape[1] / 24000:.2f} 秒")

            # 最后才返回完整的音频结果
            yield {"tts_speech": final_audio}
