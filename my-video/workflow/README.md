...
`workflow/` 是 `my-video` 的编排层，用来把下面三段链路串成一次命令：

- `voice/`: 需要时先做 TTS
- `digital_human/`: 用音频驱动数字人视频
...
```bash
cd /Users/xingdong/workspace/HD_HUMAN/my-video
.venv/bin/python -m workflow run \
  --audio voice/output/api_test.wav \
  --face 测试视频.mp4 \
  --runtime cpu \
  --with-subtitles \
  --subtitle-font-path /Users/xingdong/workspace/HD_HUMAN/font/AlibabaPuHuiTi-3-55-Regular.ttf \
  --output workflow/output/api_test_final.mp4
```
...
.venv/bin/python -m workflow run \
  ...
  --face 测试视频.mp4 \
  --runtime cpu \
  --bgm-path workflow/output/test_assets/audio_short.wav \
  --bgm-volume 0.25 \
  --output workflow/output/api_test_with_bgm.mp4
```
