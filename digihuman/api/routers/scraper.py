...
def _find_cached_video(modal_id: str) -> dict | None:
    ...
    if not modal_id or modal_id == "unknown":
        return None
    storage = get_storage()
    return storage.find_by_tag("scraper", "douyin_id", modal_id)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for p in Path(tmpdir).iterdir():
            if p.is_file():
                ...
                file_id = storage.save(p, "scraper", p.name, tags=tags)
                saved.append({
                    "file_id": file_id,
                    "filename": p.name,
...
async def list_videos():
    ...
    items = storage.list_category("scraper")
    videos = []
    for m in items:
...
async def transcribe_existing(req: DouyinExistingRequest, request: Request):
    ...
    task_id = get_task_manager().submit(
        task_type="scraper/transcribe-existing",
...
def _execute_download(task: Task, params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        storage = get_storage()
        tags = {"douyin_id": modal_id} if modal_id != "unknown" else None
        file_id = storage.save(video_path, "scraper", f"{title}.mp4", tags=tags)
